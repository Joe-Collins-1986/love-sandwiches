...
try:
    ...
    print("can't connect to google sheets:")
...
def validate_data(values):
    """
    Inside try, converts input string to int values.
...
def calculate_surplus_data(sales_row):
    ...
    """
    Could also be written like the below........
    for i in range(len(sales_row)):
        surplus_data.append(stock_row[i] - sales_row[i])
    """
...
def forcast_data(columns):
    ...
    for column in columns:
        ...
        forcast = round((sum(column) / len(column) * 1.1))
